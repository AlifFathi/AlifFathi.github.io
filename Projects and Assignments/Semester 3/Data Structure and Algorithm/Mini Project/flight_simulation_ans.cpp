...
void Queue::enQueue(Flight* f) {
    if (!isFull()) {
        int pos = (rear + 1) % size_queue;
        while (pos > front && items[(pos - 1 + size_queue) % size_queue]->fuel > f->fuel) 
        {
            items[pos] = items[(pos - 1 + size_queue) % size_queue]; 
            pos = (pos - 1 + size_queue) % size_queue;
        }
        items[pos] = f; 
        rear = (rear + 1) % size_queue;
        count++;
    } else {
        cout << "Queue is full...\n\n";
    }
